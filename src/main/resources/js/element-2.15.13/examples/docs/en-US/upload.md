...
:::demo Customize upload button type and text using `slot`. Set `queryLimit` and `on-exceed` to queryLimit the maximum number of uploads allowed and specify method when the queryLimit is exceeded. Plus, you can abort removing a file in the `before-remove` hook.
```html
<el-upload
  class="upload-demo"
  ...
  export default {
    ...
    methods: {
      ...
      handleExceed(files, fileList) {
        this.$message.warning(`The queryLimit is 3, you selected ${files.length} files this time, add up to ${files.length + fileList.length} totally`);
      },
...
Use `before-upload` hook to queryLimit the upload file format and size.
...
queryLimit | maximum number of uploads allowed | number | — | — |
on-exceed | hook function when queryLimit is exceeded | function(files, fileList) | — | - |
