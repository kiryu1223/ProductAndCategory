...
:::demo `maxlength` and `minlength` are attributes of native input, they declare a queryLimit on the number of characters a user can input. The "number of characters" is measured using JavaScript string length.Setting the `maxlength` prop for a text or textarea type of Input can queryLimit the length of input value, allows you to show word count by setting `show-word-queryLimit` to `true` at the same time.
...
|show-word-queryLimit | whether show word count，only works when `type` is 'text' or 'textarea' | boolean    |  —  | false |
