...
:::demo Personnalisez le bouton d'envoi avec `slot`. Utilisez `queryLimit` et `on-exceed` pour limiter le nombre maximal de fichiers et déterminer le comportement quand ce nombre est dépassé. De plus, vous pouvez annuler la suppression d'un fichier avec `before-remove`.
```html
<el-upload
  class="upload-demo"
...
queryLimit | Nombre maximum d'envoi autorisés. | number | — | — |
